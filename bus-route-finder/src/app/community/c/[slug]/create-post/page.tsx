...
export default function CreatePostSlugPage() {
  ...
  React.useEffect(() => {
    const loadCommunityData = async () => {
      try {
        ...
        const validation = await communityStore.validateMembershipForPostCreation(communityData.id, user.id)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newPost = await communityStore.createPost(community.id, postData, user.id)
      
      if (newPost) {
        // Navigate back to slug-based community page
