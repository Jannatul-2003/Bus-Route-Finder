...
import { useCommunityLegacyRedirect } from "@/hooks/useLegacyRedirect"
...
export default function CommunityDetailPage() {
  ...
  useCommunityLegacyRedirect(communityId)
  ...
  React.useEffect(() => {
    const loadCommunityData = async () => {
      try {
        ...
        // Check membership status if user is logged in using improved store method
        if (user && communityId) {
          setMembershipLoading(true)
          try {
            const isMemberStatus = await communityStore.getMembershipStatus(communityId, user.id)
  ...
  const handleJoin = async () => {
    try {
      ...
      setIsMember(true)
      // Community data and member count are updated in the store
    } catch (err) {
  ...
  const handleLeave = async () => {
    try {
      ...
      setIsMember(false)
      // Community data and member count are updated in the store
      // Posts are preserved as per requirements 6.4
    } catch (err) {
  ...
  const handleCreatePost = async () => {
    ...
    if (community?.name) {
      ...
      // Fallback if we can't generate slug
      console.warn('Cannot generate community slug for post creation')
    }
  }
  ...
  const handlePostStatusChange = (status: any) => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <TabsContent value="posts" className="space-y-4">
          ...
          {/* Posts are visible to all users (Requirements 4.4, 5.1, 5.5) */}
          {posts.length === 0 ? (
            <Card>
            ...
            <div className="space-y-4">
              ...
              {/* Vertical stack layout for posts (Requirements 5.1, 5.2, 5.4) */}
